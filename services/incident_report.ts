...
export class IncidentReportBuilder {
  ...
  public build(): IncidentReport {
    ...
    if (!this.report.id) {
      this.report.id = `incident-${Date.now().toString(36)}-${Math.random().toString(36).substr(2, 9)}`;
    }
...
export function incidentReportToMarkdown(report: IncidentReport): string {
  ...
  lines.push('---');
  lines.push('');
  lines.push('*Generated by Interlock v5.0.0 — The Circuit Breaker for AI Infrastructure*');
  lines.push('');
