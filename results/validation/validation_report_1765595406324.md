# Interlock Phase V Validation Report

> Evidence-driven verification of Phase V objectives

**Generated:** 2025-12-13T03:10:06.324Z
**Overall Result:** ❌ SOME TESTS FAILED

**Summary:** 2/4 tests passed. Review failures.

## Test Results

### ❌ Flapping Prevention

**Goal:** Prove hysteresis prevents instability

**Metrics:**

| Metric | Value |
|--------|-------|
| noInterlockCrashes | 15 |
| withoutHysteresisTransitions | 1 |
| withHysteresisTransitions | 1 |
| transitionReduction | 0 |
| withHysteresisUnstableTime | 17600 |

**Details:**

- No Interlock: 15 crashes
- Without Hysteresis: 1 transitions
- With Hysteresis: 1 transitions

### ✅ Incident Quality

**Goal:** Prove forensic reports are actionable

**Metrics:**

| Metric | Value |
|--------|-------|
| reportsGenerated | 5 |
| validReports | 5 |
| hasWhyOccurred | 5 |
| hasWhatPrevented | 5 |
| hasWhatToChange | 5 |

**Details:**

- Report 1: ✓ Why occurred, ✓ What prevented, ✓ What to change
- Report 2: ✓ Why occurred, ✓ What prevented, ✓ What to change
- Report 3: ✓ Why occurred, ✓ What prevented, ✓ What to change
- Report 4: ✓ Why occurred, ✓ What prevented, ✓ What to change
- Report 5: ✓ Why occurred, ✓ What prevented, ✓ What to change
- JSON format valid: true
- Markdown readable: true

### ✅ Counterfactual Survival

**Goal:** Prove Interlock prevents failure

**Metrics:**

| Metric | Value |
|--------|-------|
| controlSurvived | 0 |
| protectedSurvived | 1 |
| controlCrashPoint | 77 |
| maxLoadControl | 0 |
| maxLoadProtected | 99 |
| loadImprovement | 99 |

**Details:**

- Control (no Interlock): Crashed at step 77
- Protected: Survived
- Max survivable load - Control: 0%
- Max survivable load - Protected: 99%

### ❌ Trust Decay

**Goal:** Prove Interlock knows when it does not know

**Metrics:**

| Metric | Value |
|--------|-------|
| earlyConfidence | 85.00 |
| lateConfidence | 25.25 |
| confidenceDropPercent | 70.29 |
| confidenceDropped | 1 |
| escalatedConservatively | 0 |
| noFalseCertainty | 0 |

**Details:**

- Warning: Remained closed with only 29% confidence
- Warning: Remained closed with only 28% confidence
- Warning: Remained closed with only 27% confidence
- Warning: Remained closed with only 26% confidence
- Warning: Remained closed with only 25% confidence
- Warning: Remained closed with only 24% confidence
- Warning: Remained closed with only 23% confidence
- Warning: Remained closed with only 22% confidence
- Warning: Remained closed with only 21% confidence
- Warning: Remained closed with only 20% confidence
- Warning: Remained closed with only 20% confidence
- Warning: Remained closed with only 20% confidence
- Warning: Remained closed with only 20% confidence
- Warning: Remained closed with only 20% confidence
- Warning: Remained closed with only 20% confidence
- Warning: Remained closed with only 20% confidence
- Warning: Remained closed with only 20% confidence
- Warning: Remained closed with only 20% confidence
- Warning: Remained closed with only 20% confidence
- Warning: Remained closed with only 20% confidence
- Warning: Remained closed with only 20% confidence
- Warning: Remained closed with only 20% confidence
- Warning: Remained closed with only 20% confidence
- Warning: Remained closed with only 20% confidence
- Warning: Remained closed with only 20% confidence
- Warning: Remained closed with only 20% confidence
- Warning: Remained closed with only 20% confidence
- Warning: Remained closed with only 20% confidence
- Warning: Remained closed with only 20% confidence
- Early average confidence: 85.0%
- Late average confidence: 25.2%
- Confidence dropped: Yes
- Escalated conservatively: No
- No false certainty: No

---

## Success Criteria (v2.0 Bar)

Interlock v2.0 is complete when:

- ❌ Breaker hysteresis is evidence-based and stable
- ✅ Incident reports are post-mortem ready
- ❌ Flapping is eliminated
- ✅ Counterfactual survival advantage is demonstrated
- ❌ Trust decay is properly handled

---

*Generated by Interlock Phase V Validation Suite*